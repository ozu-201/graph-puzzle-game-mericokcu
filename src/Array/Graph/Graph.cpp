...
namespace array{
    ...
    void Graph::search(std::vector<std::string>& words,std::string & start, std::string & target) {
        ...
        for(int i=0; i<words.size(); i++){
            ...
            if(target_index == -1 || start_index == -1){
                std::cout << "Start word or target word wasn't found in the dictionary";
                return;
            }
            ...
            while(!queue.isEmpty()){
                ...
                for(int toNode = 0; toNode < vertexCount; toNode++){
                    if(edges[currentNode][toNode] > 0 && !visited[toNode]){
                        visited[toNode] = true;
                        parent[toNode] = currentNode;
                        queue.enqueue(Element(toNode));

                        if(toNode == target_index){
